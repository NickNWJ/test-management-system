...
import SuccessDialog from '../pages/UiElements/Success';
import ErrorDialog from '../pages/UiElements/Error';
import ConfirmationDialog from '../pages/UiElements/Confirmation';
...
const DropdownUser = () => {
  ...
  const { user, loginTime, handleLogout } = useAuth();
  ...
  useEffect(() => {
    ...
    return () => document.removeEventListener('keydown', keyHandler);
      ...
      const handleOpenLogoutConfirmationModal = () => setIsOpenLogoutConfirmationModal(true);
      const handleCloseLogoutConfirmationModal = () => setIsOpenLogoutConfirmationModal(false);
      ...
      }, [navigate, showLogoutSuccessDialog])
      ...
      const handleLogoutClick = async() => {
        ...
        if (shouldLogout) {
          ...
          // navigate("/login");   
          try {
              const response = await axios.post('http://localhost:8888/api/users/logout');
  ...
  return (
    ...
    <div className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-base font-bold">{user.Role}</span>
        </span>
      ...
      >
        ...
        <button
          ...
          className="flex items-center hover:bg-gray-2 gap-3.5 py-4 px-6 text-sm font-medium duration-300 ease-in-out hover:text-primary lg:text-base">
          <svg
            className="fill-current"
            width="22"
            height="22"
    ...
    { showLogoutSuccessDialog && <SuccessDialog message={"Logged Out Successfully."} />}
    </>
  );
};
