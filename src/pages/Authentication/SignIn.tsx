...
const SignIn = () => {
  ...
  const { user, setUser, setLoginTime, handleLogin } = useAuth();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try{
      const response = await axios.post('http://localhost:8888/api/users/login', { Email: email, Password: password });

      if (response.status === 200) {
        console.log(response.data.token);
        setCookie('token', response.data.token, { path: '/', secure: true, sameSite: 'none', httpOnly: true });
        console.log('res data -> ', response.data);
        setCookie('user', response.data.user, { path: '/', secure: true, sameSite: 'none', httpOnly: true });
        console.log("Token in Cookies -> ", cookies);
        handleLogin(response.data.user, response.data.token);
        setLoginTime(moment().format('DD MMMM YYYY hh:mm:ssA'));
        // Set the state variables to show the success dialog and navigate to the home page
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap items-center">
          <div className="hidden w-full xl:block xl:w-1/2">
            <div className="py-17.5 px-26 text-center">
              <Link className="mb-5.5 flex w-full justify-center text-center items-center" to="/">
                <img className="hidden dark:flex w-8" src={Logo} alt="Logo" />
                <img className="dark:hidden w-8" src={Logo} alt="Logo" />
                <span className="flex ml-2 w-8 font-bold text-2xl">ATest</span>
              </Link>

              <p className="2xl:px-20">
                A Web-Based GUI of Automation Testing
              </p>
            ...
            <div className="w-full p-4 sm:p-6 xl:p-8">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-5">
                  <button
                    type="submit"
                    className="w-full cursor-pointer text-whiten rounded-lg border border-primary bg-primary p-4 transition hover:bg-opacity-90"
                  >Sign In</button>
                </div>
      ...
      {showSuccessDialog && (
        <SuccessDialog
          message={`Welcome to ATest, ${user.Name}.`}
